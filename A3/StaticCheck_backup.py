...
class Checker:
    ...
    def checkUndeclared(visibleScope, name, kind, notGlobal=False):
        ...
        # res = Checker.utils.lookup((str(name), type(kind)), scope, lambda x: x.toTuple())
    ...
    def matchType(patternType, paramType):
        ...
        if type(patternType) == ArrayPointerType and type(paramType) in [ArrayType, ArrayPointerType]:
            ...
            return Checker.matchType(patternType.eleType, paramType.eleType)
...
class StaticChecker(BaseVisitor, Utils):
    ...
    def visitBinaryOp(self, ast: BinaryOp, params):
        ...
        if len(params) == 2:
            ...
            # ['+', '-', '*', '/', '%', '>', '<', '>=', '<=']
            if ExpCheck.isOpForNumber(op):
                if ExpCheck.isNaNType(lType) or ExpCheck.isNaNType(rType):
                    raise TypeMismatchInExpression(ast)
                elif op in ['+', '-', '*', '/']:
                    if type(lType) == IntType and type(rType) == IntType:
                        return (IntType(), False)
                    else:
                        return ExpCheck.mergeNumberType(lType, rType)
                elif op == '%':
    ...
    def visitUnaryOp(self, ast: UnaryOp, params):
        ...
        if len(params) == 2:
            ...
            if (op == '-' and ExpCheck.isNaNType(expType)) or (op == '!' and type(expType) is not BoolType):
                raise TypeMismatchInExpression(ast)
    ...
    def visitArrayCell(self, ast: ArrayCell, params):
        if len(params) == 2:
            ...
            scope, retType, inLoop, funcName = params
            arrType = self.visit(ast.arr, params)[1]
            idxType = self.visit(ast.idx, params)[0]
            if not(type(idxType) is IntType) or not(type(arrType) in [ArrayType, ArrayPointerType]):
                raise TypeMismatchInExpression(ast)
            return (ast, None)
